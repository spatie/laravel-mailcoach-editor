...
export default class CodeTool {
     ...
     * @property {string} theme - the theme
     */
    ...
    drawView() {
        const wrapper = document.createElement('div'),
            ...
            theme = document.createElement('select');
        ...
        language.classList.add(this.CSS.input);
        language.value = this.data.language;
        language.placeholder = 'Language';

        theme.classList.add(this.CSS.input, 'theme-selector');
        ...
        extraWrap.appendChild(language);
        extraWrap.appendChild(theme);

        extraWrap.classList.add('grid', 'grid-cols-2', 'gap-4', 'mt-4')
        ...
        theme.appendChild(placeholderOption);
        ...
        for (let i = 0; i < themeOptions.length; i++) {
            ...
            const themeOption = themeOptions[i];

            if (typeof themeOption === 'string') {
                option.setAttribute('value', themeOption);
                option.textContent = themeOption;
            } else {
                option.setAttribute('value', themeOption.value);
                option.textContent = themeOption.label;
            }
        ...
        this.nodes.language = language;
        this.nodes.theme = theme;
    ...
    set data(data) {
        ...
        if (this.nodes.language) {
            this.nodes.language.value = data.language;
        }

        if (this.nodes.theme) {
            this.nodes.theme.value = data.theme;
        }
    }
